...
public class FriendRequestPop {
	...
	public void showFriendRequestMainPop() {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				pw.dismiss();
				//showZhuanjiaDetailsPop(datas.get(position).getMessage_id(), datas.get(position).getMessage_img(), datas.get(position).getMessage_content());
	...
	public void loadFriendInviteList(){
		...
		FriendInviteAdapter adapter=new FriendInviteAdapter(msgs, activity,token);
	...
	public void showZhuanjiaDetailsPop(final String title, final String img_url, final String content) {
		...
		Glide.with(activity).load(img_url).placeholder(R.drawable.h_xiqtp).transform(new GlideRoundTransform(activity, 80)).fitCenter().into(ivIcon);
